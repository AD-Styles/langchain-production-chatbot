...
이번 프로젝트는 앞서 만든 챗봇들이 가진 한계를 정면으로 마주하고 다듬는 과정이었습니다. 1번에서 외부 시스템과 연결할 줄 아는 챗봇을 만들었고, 2번에서 회사 문서를 검색해서 답하는 챗봇까지 갔는데, 실제로 그 챗봇을 회사 시스템에 그대로 넣어 보려고 하니 빈 곳이 너무 많이 보였습니다. 응답이 자유로운 문장으로만 나와서 자동으로 처리하기가 어려웠고, 이전 대화를 전혀 기억하지 못해서 매번 처음부터 다시 설명해야 했고, 대화가 조금만 길어져도 토큰이 폭증해서 비용이 부담스러웠습니다. 이 세 가지를 한 번에 풀어 보자는 게 이번 작업의 출발점이었습니다.

가장 먼저 손댄 부분은 응답 형식이었습니다. Pydantic 으로 응답 모양을 미리 정해 두면 LLM 이 그 안에서만 답을 만들도록 강제할 수 있다는 점이 처음 들었을 때는 약간 의심스러웠습니다. "모델이 정말 그 규칙을 지킬까?"라는 의문이 있었는데, 실제로 `with_structured_output` 한 줄로 카테고리와 긴급도가 항상 미리 정한 값 중 하나로만 나오는 걸 보고 나서야 이게 단순한 형식 통일을 넘어 안전장치 역할을 한다는 점을 이해했습니다. 특히 인상 깊었던 건 `Literal["low", "medium", "high", "critical"]` 같은 표현이었습니다. 모델이 "매우 급함" 같은 자유 표현을 쓰지 못하게 미리 막아 두는 셈인데, 사후에 정규식으로 파싱하던 방식과 비교하면 훨씬 견고했습니다. 타입이라는 게 사후 검증 도구가 아니라 사전 강제 도구가 될 수 있다는 점이 새로웠습니다.

체크포인터 부분은 의외로 코드 변화는 적었지만 챗봇의 인상이 가장 크게 바뀐 부분이었습니다. `InMemorySaver` 객체 하나를 만들어서 `create_agent(checkpointer=...)` 에 넣고, 호출할 때 `config={"configurable": {"thread_id": "..."}}` 만 같이 넘기면 끝이었습니다. 코드는 두 줄이지만, 그 전후의 사용자 경험은 완전히 다른 챗봇처럼 느껴졌습니다. 이름을 알려 주면 다음 턴에서 그 이름을 부르고, 이전에 받은 데이터시트 얘기를 이어서 할 수 있고 — 사람을 상대하는 느낌이 갑자기 생겼습니다. 다만 한 가지 알아 둬야 할 점은, 기억이 자동으로 되는 게 아니라 `thread_id` 라는 키 단위로 관리된다는 사실이었습니다. 처음에는 "체크포인터만 켜면 알아서 다 기억하나 보다" 하고 넘어갔는데, 실제로는 같은 thread_id 를 매번 같이 넘겨야 같은 대화로 이어지고, thread_id 가 바뀌면 새 대화가 시작됩니다. 이 구조가 멀티 사용자 환경에서는 오히려 장점이 됩니다. 사용자 ID 를 그대로 thread_id 로 쓰면 자연스럽게 사용자별 대화 분리가 풀리기 때문입니다.

요약 미들웨어는 처음에는 "굳이 필요할까?" 싶었습니다. 짧은 시연 단계에서는 토큰이 별로 늘지 않아서 체감이 잘 안 됐기 때문입니다. 그런데 30턴 정도 일부러 길게 늘려 보면서 그래프로 그려 보니 누적 토큰이 4,000을 금방 넘어가는 게 보였고, 그때부터 매 턴마다 같은 정보를 모델에게 다시 보내는 식이 되어 비용이 부담스러워지는 게 분명했습니다. `SummarizationMiddleware(trigger=('tokens', 4000), keep=('messages', 20))` 한 줄을 넣어 두니, 임계치를 넘는 순간부터 오래된 턴은 짧은 요약본으로 대체되고 최근 20개 턴만 원문 그대로 남았습니다. 가장 마음에 들었던 부분은 `keep` 옵션이 있어서 "최근 맥락은 절대 손대지 않는다"는 안전장치를 둘 수 있다는 점이었습니다. 단순한 비용 절감이 아니라, 모델이 컨텍스트 윈도우를 초과해서 답변 자체를 못 만드는 상황을 막아 주는 방어선 역할이었습니다.

세 가지를 다 합쳐 놓고 보니, 시연용 챗봇과 운영용 챗봇이 실제로 어떤 점에서 다른지 처음으로 손에 잡히는 느낌이 들었습니다. 한 사람이 한 번 써 보고 끝나는 시연에서는 분류도 기억도 요약도 굳이 필요 없습니다. 그런데 여러 사용자가 며칠에 걸쳐 동시에 쓰는 환경을 가정하면, 이 네 가지(정형 응답·기억·요약·격리)가 한꺼번에 갖춰져야 비로소 "쓸 수 있다"는 말이 성립합니다. 한두 가지만 빠져 있어도 어디선가 무너지는 구조라는 점이 인상 깊었습니다. 다음 단계로는 이번에 만든 챗봇을 실제로 FastAPI 같은 서버 프레임워크 위에 올려서, 사용자별 thread_id 를 자동으로 발급하고, 분류 결과를 메일·티켓 시스템과 연동하는 데까지 가 보고 싶습니다. 이번에 만든 세 축이 그 다음 단계의 단단한 받침대가 되어 줄 것 같습니다.
