...
핵심 질문은 다음의 세 가지 입니다. **Pydantic 으로 LLM 출력을 강제하면 회사 시스템에 자동으로 꽂아 넣을 수 있는가 / 같은 챗봇이 여러 고객을 동시에 응대하면서 각자의 대화를 섞이지 않게 기억할 수 있는가 / 30턴 넘는 긴 대화에서도 토큰 비용이 폭주하지 않게 막을 수 있는가** — 이 세 가지를 코드로 검증하는 게 이 포트폴리오의 목표입니다.
